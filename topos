   ...
   fun exponential(a,b) =
	let val fs = function_space(a,b)
	    ...
	    val eval = set_mor(fs_X_a,
		               fn pr => let val function l = proj_fs OF pr
                                            and z = proj_a OF pr
...
(* module logic_in_topoi *)
   ...
   fun ToposFALSE (T as topos(C,lim,colim,_,_)) =
	   let val (init,_) = initial(cocomplete_cat(C,colim))
	       val m = terminal_mor(complete_cat(C,lim))(init)
           in character(T)(m)
           end;

   fun ToposNOT(T) = character(T)(ToposFALSE T);

   fun ToposAND(T as topos(C,lim,colim,_,sc) ) =
   ...
   fun ToposOR(T as topos(C,lim,colim,_,((truvals,truth),_))) =
	let val lC = complete_cat(C,lim)
   ...
   fun ToposIMPLY(T as topos(C,lim,colim,_,( (truvals,truth),_) )) =
	let val lC = complete_cat(C,lim)
	    val ((prod,r,s),pr_univ) = product(lC)(truvals,truvals) 
   ...
   fun ToposFOR_ALL T a = character T (name T (true_ T a));

   fun ToposEXISTS (T as topos(C,lim,_,_,_)) a =
